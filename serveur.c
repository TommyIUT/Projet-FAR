...
int port;
struct sockaddr_in server_addrfile, csin;
FILE *fp ;
int sockacceptfile,sockacceptfile;
...
int send_file(int uid) {
    struct dirent *dir;
	char buffer[BUFFER_SZ] = ""; // Buffer pour stocker les noms de fichiers

	DIR *d = opendir(".");
	if (d) {
		while ((dir = readdir(d)) != NULL) {
			strcat(buffer, dir->d_name); // Concaténer le nom de fichier au buffer
			strcat(buffer, "\n"); // Ajouter une nouvelle ligne après chaque nom de fichier
		}
		closedir(d);
	}
...
void receive_file(char *s,int uid) {
		...
		char* data = malloc(sizeof(char) * (strlen(s)+1));
		FILE *fp ;
		...
		int sockfile = accept(sockacceptfile, (struct sockaddr*)&csin, &crecsize);

		//recoit le nom du fichier
		bytes_received = recv(sockfile, buffer, SIZE, 0);
		strcpy(filename,buffer);

		//ouvre fichier
		fp = fopen(filename,"wb");
		
		while ((bytes_received = recv(sockfile, buffer, SIZE, 0)) > 0) {
...
int main(int argc, char **argv){
	...
	char *ip = "127.0.0.1";
	port = atoi(argv[1]); // port a ecouter
	int option = 1;
	...
	file_addr.sin_addr.s_addr = inet_addr(ip);
	file_addr.sin_port = (port+1);
	...
	if(bind(listenfile, (struct sockaddr*)&file_addr, sizeof(file_addr)) < 0) {
		perror("ERROR: Socket binding failed");
		return EXIT_FAILURE;
	}

	// met le socket en mode ecoute
	if (listen(listenfile, 10) < 0) {
	...
	// socket pour accepter fichier
	sockacceptfile = socket(AF_INET, SOCK_STREAM, 0);
	server_addrfile.sin_family = AF_INET;
	server_addrfile.sin_port = htons(port+1);
	server_addrfile.sin_addr.s_addr = htonl(INADDR_ANY);
	bind(sockacceptfile, (struct sockaddr*)&server_addrfile, sizeof(server_addrfile));
	listen(sockacceptfile,5);
